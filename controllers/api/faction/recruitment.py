...
def invite_recruit(*args, **kwargs):
    ...
    recruit: RecruitModel = utils.last(RecruitModel.objects(Q(tid=user) & Q(factionid=kwargs['user'].factionid)))
    user: UserModel = utils.first(UserModel.objects(tid=user))
...
def message_send(*args, **kwargs):
    ...
    recruit: RecruitModel = utils.last(RecruitModel.objects(Q(tid=receiver_data['player_id']) & Q(factionid=kwargs['user'].factionid) & Q(recruiter=kwargs['user'].tid)))
