...
void scheduler::parse_args(int argc, char *argv[], scheduler::config &config_) {
  cxxopts::Options options(
      "scheduler", "A high performance Torn API call scheduler and proxy");

  options.add_options()
      // Options
      ("v,verbose", "verbose output",
       cxxopts::value<bool>()->default_value("false"))("dump", "dump config")(
          "h,help", "print usage")("license", "print license")
      // Positional arguments
      ("path", "path to the socket",
       cxxopts::value<std::string>()->default_value("/tmp/scheduler.sock"));
