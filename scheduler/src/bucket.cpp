...
scheduler::insertion_status
scheduler::RequestBucket::try_emplace(scheduler::Request &request_) {
  ...
  if (request_.request_type == nice_type::user_request or
    ...
    if (has_maxed_requests) {
      ...
      std::sort(
          bucket_requests.begin(), bucket_requests.end(),
...
scheduler::RequestBucket
scheduler::insert_request(scheduler::Request &request_) {
  request_buckets.try_emplace(request_.user_id, scheduler::RequestBucket());
  scheduler::RequestBucket bucket_ =
      request_buckets[request_.user_id]; // TODO: Use the iterator from
