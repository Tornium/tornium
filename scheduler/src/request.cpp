...
std::optional<scheduler::Request>
scheduler::parse_request(char *data_, const size_t &bytes_received,
...
bool scheduler::enqueue_request(scheduler::Request &request_) {
  ...
  if (requests_map.count(request_.endpoint_id) > 0) {
    ...
    for (auto i = existing_keys_range.first; i != existing_keys_range.second;
         i++) {
...
void scheduler::remove_request(std::string request_key) {
  requests_map.erase(request_key);
}
