...
bool compare_request_priority(const scheduler::Request *r1,
                              const scheduler::Request *r2) {
...
void scheduler::queue_request(scheduler::Request &request_) {
  std::vector<scheduler::Request *> &user_request_queue =
      request_queue.try_emplace(request_.user_id, std::vector<Request *>{})
          .first->second;

  auto sorting_iterator =
      std::lower_bound(user_request_queue.begin(), user_request_queue.end(),
                       &request_, compare_request_priority);
