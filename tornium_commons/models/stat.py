...
class Stat(BaseModel):
    ...
    def generate_chain_list(
        ...
        if sort == "timestamp":
            ...
            query = 'select "t1"."id", "t1"."tid_id", "t1"."time_added", "t1"."battlescore", "t2"."name", "t2"."level", "t2"."last_refresh", "t2"."faction_id", "t2"."status", "t2"."last_action", "t3"."name"  from (select * from (select distinct on ("t1"."tid_id") * from "stat" as "t1" where ("t1"."added_group" = 0) or ("t1"."added_group" = %s) order by "t1"."tid_id", "t1"."time_added" desc) t1 where ("t1"."battlescore" >= %s) and ("t1"."battlescore" <= %s)) t1 join "user" as "t2" on "t2"."tid" = "t1"."tid_id" left join "faction" as "t3" on "t3"."tid" = "t2"."faction_id" where "t2"."level" is not null order by random() limit %s'
        else:  # Sorted by respect
            query = f'select "t1"."id", "t1"."tid_id", "t1"."time_added", "t1"."battlescore", "t2"."name", "t2"."level", "t2"."last_refresh", "t2"."faction_id", "t2"."status", "t2"."last_action", "t3"."name"  from (select * from (select distinct on ("t1"."tid_id") * from "stat" as "t1" where ("t1"."added_group" = 0) or ("t1"."added_group" = %s) order by "t1"."tid_id", "t1"."time_added" desc) t1 where ("t1"."battlescore" >= %s) and ("t1"."battlescore" <= %s)) t1 join "user" as "t2" on "t2"."tid" = "t1"."tid_id" left join "faction" as "t3" on "t3"."tid" = "t2"."faction_id" where "t2"."level" is not null order by ((log("t2"."level") + 1) / 4 * greatest(1 + 8 / 3 * ("t1"."battlescore" / {int(invoker.battlescore)}), 3)) desc, "t1"."time_added" desc limit %s'
